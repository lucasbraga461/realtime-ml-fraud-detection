...
API_HOST = "http://10.0.1.165:8502"
AUTH_FILE_PATH = '/root/Documents/config/auth_flask.json'
DATA_FILE_PATH = '/root/Documents/data/european_creditcard.csv'
        ...
        with self.client.post(
            ...
            if response.status_code == 200:
                try:
                    ...
                    required_keys = {'score', 'prediction', 'threshold'}
                    if required_keys.issubset(response_json.keys()):
                        response.success()
                        # print(f"Response: {response_json}")
                    else:
                        missing_keys = required_keys - response_json.keys()
                        response.failure(f"Missing keys in response JSON: {missing_keys}")
