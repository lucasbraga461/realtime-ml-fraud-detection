...
applications=(
    "flask-cnn-app-py39:deep_learning/flask-cnn-app-py39"
    "flask-cnn-app:deep_learning/flask-cnn-app"
    "flask-lstm-app-py39:deep_learning/flask-lstm-app-py39"
    "flask-lstm-app:deep_learning/flask-lstm-app"
    "flask-transformers-app-py39:deep_learning/flask-transformers-app-py39"
    "flask-transformers-app:deep_learning/flask-transformers-app"
    "flask-stacking-app:machine_learning/flask-stacking-app"
    "flask-stacking-dl-app:machine_learning/flask-stacking-dl-app"
    "flask-lgbm-app:machine_learning/flask-lgbm-app"
    "flask-lgbm-app_py39:machine_learning/flask-lgbm-app_py39"
    "flask-logreg-app:machine_learning/flask-logreg-app"
    "flask-xgboost-app:machine_learning/flask-xgboost-app"
)

# Infrastructure types
infrastructures=("ec2" "ecs" "ecs_alb")
...
test_application() {
    ...
    # Use dynamic deployment system
    echo -e "${infra_type}\n${app_name}\ny" | timeout 1800 ./dynamic_deployment_system.sh || {
        ...
        "ecs")
            ...
            if [ ! -z "$cluster_name" ]; then
                ...
                if [ "$task_arn" != "None" ] && [ ! -z "$task_arn" ]; then
                    public_ip=$(aws ecs describe-tasks --cluster "$cluster_name" --tasks "$task_arn" --query 'tasks[0].attachments[0].details[?name==`networkInterfaceId`].value' --output text 2>/dev/null | xargs -I {} aws ec2 describe-network-interfaces --network-interface-ids {} --query 'NetworkInterfaces[0].Association.PublicIp' --output text 2>/dev/null || echo "")
    ...
    print_status "Cleanup completed for $app_name on $infra_type"
}
...
main() {
    ...
    # Test each application on each infrastructure
    for app_entry in "${applications[@]}"; do
        IFS=':' read -r app_name app_path <<< "$app_entry"
        
        for infra_type in "${infrastructures[@]}"; do
            test_application "$app_name" "$app_path" "$infra_type"
